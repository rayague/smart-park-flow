...
import { X, Loader2, CheckCircle2, AlertCircle, Edit, Zap } from "lucide-react"
...
export function EditParkingModal({ parking, onClose }: EditParkingModalProps) {
    ...
    // Initialize form when parking changes
    React.useEffect(() => {
        if (parking) {
            setForm({
                name: parking.name,
                ...
                openingHours: {
                    open: parking.openingHours.open,
                    close: parking.openingHours.close
                },
            })
            setStatus("idle")
            setErrorMsg("")
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setErrorMsg("")
        try {
            await updateParking(parking.id, form)
            setStatus("success")
